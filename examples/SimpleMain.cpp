...
int main(int argc, char* argv[])
{
  ...
  parameters.MaxNumOfPoints[0] = 10000;
  ...
  std::string input_path = "D:\\input_data_new_5_1_temp.txt";
	std::string output_filename = "D:\\output_new_5_1_temp.csv";
...
//    convert_param = need_convert(atoi(argv[3]));
  ...
  problem = new ProblemFromFunctionPointers(parameters.Dimension, // размерность задачи
    ...
    std::vector<std::function<double(const double*)>>(1, [&input, &c_data](const double* y)
      {
				input.initial_k_base[0].assign(y, y + parameters.Dimension);
				double res = DirectProblemSolver::calc_y_and_get_error(input.initial_k_base[0], input.y_exp_conv[0], c_data[0].y, c_data[0].W, input, input.degrees);
				return res;
      }) // критерий 
  );
  ...
  if (solver.Solve() != SYSTEM_OK)
	...
	input.initial_k_base[0].assign(resY, resY + parameters.Dimension);

	c_data[0].k = input.initial_k_base[0];
